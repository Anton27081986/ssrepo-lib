...
export class ModalService {
  ...
  public open<T>(params: PopupParams<T>): ModalRef<T> {
    ...
    const popoverRef = new ModalRef<T>(overlayRef, params.content, params.data, params.type);

    const injector = this._createInjector(popoverRef, this.injector);

    overlayRef.attach(new ComponentPortal(GenericPopupComponent, null, injector));
  ...
  private _getOverlayConfig<T>(params: PopupParams<T>): OverlayConfig {
    let backdropClass: string[] = ['ss-lib-overlay-backdrop'];

    if (params.isDarkOverlay) {
      backdropClass.push('ss-lib-overlay-backdrop--dark')
    }
    let panelClass: string[] = ['ss-lib-popover-root'];
