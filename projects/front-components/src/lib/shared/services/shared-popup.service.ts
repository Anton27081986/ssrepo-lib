...
import {ModalRef} from "../models/utils/modal.ref";
...
import {IConfirmData, ILightBoxData} from '../models';
...
export class SharedPopupService {
   ...
   */
  public openModal<T>(content: PopupContent, data: T, isDarkOverlay: boolean = true, size: string, isBackDropClick: boolean = true): ModalRef<T> {
    ...
    if (isBackDropClick) { // на случай если появится необходимость закрытия по клику
  ...
  public openConfirmModal(
    ...
    isBackDropClick: boolean = false): ModalRef<IConfirmData> {
